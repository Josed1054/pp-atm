...
export default function ATMButtons({
  ...
  return (
    <div
      className={clsx(
        "grid grid-rows-5 grid-cols-1 items-center gap-3 row-start-2 w-14 h-full border-y-4 border-transparent",
    ...
    >
      ...
      {buttons.map((button, index) => (
        <div key={`${button.label}-${index}`} className="flex items-center">
          ...
          <Button
            className="w-3/4 h-6 bg-[#C1C1C1] rounded-md border-y-2 border-b-[#9B9B9B] border-t-[#D9D9D4] hover:bg-slate-300"
